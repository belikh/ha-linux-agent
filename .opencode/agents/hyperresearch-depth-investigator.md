...
description: "Use this agent in Layer 3 of the hyperresearch deep research pipeline. Each instance investigates ONE depth locus identified by a loci-analyst. The agent reads existing vault sources relevant to the locus, fetches new sources as needed (via the hyperresearch-fetcher subagent), and writes ONE interim report note summarizing what it learned. Spawn one depth-investigator per locus, in parallel. Synthesizing a narrow-but-deep question requires real reading comprehension."
mode: subagent
hidden: true
permission:
  task:
...
1. **Start with the vault.** Before fetching anything new, read the notes
   ...
   `/home/io/projects/hyperresearch-opencode/.venv/bin/hyperresearch note show <id1> <id2> <id3> --json`
...
3. **Fetch new sources via the fetcher subagent.** Do NOT call
   `/home/io/projects/hyperresearch-opencode/.venv/bin/hyperresearch fetch` directly. Delegate to `hyperresearch-fetcher` via the
...
5. **Read the fetched sources.** Use `/home/io/projects/hyperresearch-opencode/.venv/bin/hyperresearch note show <id> -j`. Quote
...
6. **Write ONE interim report note.** This is your single deliverable.
   ...
   **BEFORE calling `note new`**, check if an interim note for this locus
   ...
   ```bash
   /home/io/projects/hyperresearch-opencode/.venv/bin/hyperresearch note list --tag locus-<locus-name> --type interim --all --json
   ```
...
```bash
/home/io/projects/hyperresearch-opencode/.venv/bin/hyperresearch note new "Interim report — <locus name>" \
  --tag <corpus_tag> \
