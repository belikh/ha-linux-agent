...
repository SOURCES (`/home/io/projects/hyperresearch-opencode/.venv/bin/hyperresearch repo wiki`, `/home/io/projects/hyperresearch-opencode/.venv/bin/hyperresearch repo map`);
...
- **repo_source_note_id**: the vault note id of the repository source
  ...
  wrote). Read it with `/home/io/projects/hyperresearch-opencode/.venv/bin/hyperresearch note show <id> -j`.
...
1. **Check for an existing analysis.** Search the vault first:
   ```bash
   PYTHONIOENCODING=utf-8 /home/io/projects/hyperresearch-opencode/.venv/bin/hyperresearch note list --tag <vault_tag> --type source-analysis --all --json
   ```
...
2. **Read the repository source note.** Pull the wiki / map body:
   ```bash
   PYTHONIOENCODING=utf-8 /home/io/projects/hyperresearch-opencode/.venv/bin/hyperresearch note show <repo_source_note_id> -j
   ```
   For a DeepWiki note, the per-page child notes (parented to the wiki
   note) are the natural reading order — list them with
   `/home/io/projects/hyperresearch-opencode/.venv/bin/hyperresearch note list --parent <repo_source_note_id> --json` and
...
3. **Enrich (bounded, 3-6 calls max).** The wiki/map is a skeleton —
   ...
   - For a GitHub repo: `/home/io/projects/hyperresearch-opencode/.venv/bin/hyperresearch repo ask <owner/repo> --question "..." -j`
   ...
   - For a local checkout: `/home/io/projects/hyperresearch-opencode/.venv/bin/hyperresearch repo map <path> --top 20 -j` if no
...
5. **Create the analysis note:**
   ```bash
   PYTHONIOENCODING=utf-8 /home/io/projects/hyperresearch-opencode/.venv/bin/hyperresearch note new "Repository Analysis — <short name>" \
