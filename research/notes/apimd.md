...
created: '2026-09-02T13:50:21.690948Z'
updated: '2026-09-05T10:51:22.325743Z'
source: https://raw.githubusercontent.com/ipetkov/crane/master/docs/API.md
source_domain: raw.githubusercontent.com
fetched_at: '2026-09-02T13:50:21.688763Z'
...
content_type: docs
deprecated: false
summary: 'Crane API reference: cargoTest is ''cargoTest :: set -> drv'' which sets
  buildPhaseCargoCommand to ''cargo test --profile release'' (CARGO_PROFILE env can
  alter or empty-quote the profile) and pnameSuffix ''-test''; optional attrs cargoExtraArgs
  (default ''--locked'') and cargoTestExtraArgs (default '''') are appended to the
  invocation, and ALL other derivation attributes delegate to mkCargoDerivation, which
  passes nativeBuildInputs/buildInputs straight through to stdenv.mkDerivation (with
  cargo, rustc, rsync, zstd and crane''s hooks auto-appended as native inputs) — so
  adding mosquitto and dbus to nativeBuildInputs in commonArgs gives tests access
  to those binaries in the check phase, with pre/postCheck hooks available. cargoClippy
  sets ''cargo clippy --profile release'' with cargoClippyExtraArgs (default ''--all-targets'',
  e.g. deny specific lints like -- -D warnings) and auto-appends clippy as a native
  build input; both consume cargoArtifacts from buildDepsOnly for incremental caching.'
---
