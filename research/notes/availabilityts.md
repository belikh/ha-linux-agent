...
deprecated: false
summary: 'Zigbee2MQTT availability.ts extension source (master, the production-grade
  publisher-side availability supervisor this design can learn from). Core algorithm:
  per-device timeout from lastSeen (active vs passive devices get different defaults);
  resetTimer schedules a ping at (timeout + 1s + random jitter in [0, max_jitter))
  * backoff, where jitter = Math.random() * getMaxJitter(device) and backoff multiplies
  ×1.5 after a failed ping of an available device, ×2 when already unavailable — the
  code comment states ''results in backoffs: *1.5, *3, *6, *12... (with default timeout:
  10, 15, 30, 60, 120)''. Ping failures grow pingBackoffs map; pause_on_backoff_gt
  puts a device into availability sleep to stop ever-growing timers; MAX_TIMEOUT=2147483647
  caps setTimeout (Node 32-bit delay). Ping queue is serial (one executing at a time,
  2s gap between pings, 2 attempts with 3s retry gap if device was previously available).
  Availability publishes are RETAINED QoS 1 JSON {state:''online''|''offline''} on
  <friendlyName>/availability; publishes are deduped via lastPublishedAvailabilities
  map. On reconnect (device announces or transitions offline→online) a debounced (2s)
  retrieveState re-reads state/brightness/colour via RETRIEVE_ON_RECONNECT converters.
  On entity rename, the old availability topic is cleared by publishing an empty retained
  payload.'
---
...
private readonly lastPublishedAvailabilities = new Map();
/** Mapped by IEEE address */
private readonly pingBackoffs = new Map();
/** IEEE addresses, waiting for last seen changes to take them out of "availability sleep" */
private readonly backoffPausedDevices = new Set();
/** Mapped by IEEE address */
private readonly retrieveStateDebouncers = new Map void>();
...
private async pingQueueExecuteNext(): Promise {
...
private async publishAvailabilityForAllEntities(): Promise {
...
private async publishAvailability(entity: Device | Group, logLastSeen: boolean, forcePublish = false, skipGroups = false): Promise {
...
@bind private async onLastSeenChanged(data: eventdata.LastSeenChanged): Promise {
