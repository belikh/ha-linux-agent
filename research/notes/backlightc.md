...
deprecated: false
summary: 'Linux drivers/video/backlight/backlight.c (torvalds master): brightness_store()
  delegates to backlight_device_set_brightness(), which sets props.brightness under
  ops_lock, calls backlight_update_status(), then ALWAYS calls backlight_generate_event()
  -> kobject_uevent_env(KOBJ_CHANGE, SOURCE=sysfs) + sysfs_notify(&bd->dev.kobj, NULL,
  "actual_brightness"). So sysfs_notify on actual_brightness fires after every successful
  sysfs brightness write (and via backlight_force_update for hotkey changes) — poll(2)
  POLLPRI on actual_brightness is the kernel-supported event channel for brightness
  changes; uevent KOBJ_CHANGE is the second channel.'
---
