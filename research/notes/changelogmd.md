---
title: go-hass-agent CHANGELOG.md (full v4.3.1→v14.15.1 history)
id: changelogmd
tags:
...
summary: 'AUDITED ABSENCE + release history: go-hass-agent''s full CHANGELOG (11,224
  words, v4.3.1→v14.15.1, releases through 2026-08-09) contains ZERO occurrences of
  ''select'' (entity type), ''state sync'', or ''NixOS''. The only two ''sync'' hits
  are unrelated: a 2024 sensor-registry disabled-state fix (cc8d89c) and swapping
  golang.org/x/sync/errgroup for sourcegraph/conc/pool. MQTT controls remain one-way:
  35 MQTT / 33 control mentions never pair with state synchronisation. Current v14.x
  release cadence ~monthly (14.10.x Mar 2026 → 14.15.1 Aug 2026): volume controls,
  GPU memory sensors, ddcutil backlight, pw-dump tolerance, capability-error clarity,
  disabled-preference short-circuits. Confirms the incumbent gap persists into the
  current era: no select-with-state-sync controls, no NixOS-specific work.'
---
