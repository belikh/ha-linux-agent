---
title: core/homeassistant/components/mqtt/abbreviations.py at dev · home-assistant/core
  · GitHub
id: corehomeassistantcomponentsmqttabbreviationspy-at-dev-home-assistantcore-github
tags:
...
content_type: code
deprecated: false
summary: 'HA core abbreviations.py (dev, 297 lines) — the ground-truth mapping table
  of every legal abbreviated MQTT discovery key (cited directly by discovery.py).
  Agent-relevant entries: avty (availability), avty_mode, avty_t (availability_topic),
  avty_tpl, cmd_t (command_topic), stat_t (state_topic), uniq_id (unique_id), val_tpl
  (value_template), name, dev (device), dev_cla (device_class), ent_cat (entity_category),
  en (enabled_by_default), e (encoding), obj_id? (object_id), pl_off/pl_on (payload_off/on),
  qos, ret (retain), t (topic), ''~'' base, sn (serial_number), hw (hw_version), sw
  (sw_version), mf (manufacturer), mdl (model), cu (configuration_url), cmps (components),
  o (origin) with name/url/sw_version/support_url subkeys. Practical use for ha-linux-agent:
  emitting abbreviated keys shrinks discovery payloads measurably across a fleet (dozens
  of entities per host x N hosts), and the table is the definitive list to validate
  agent-generated payloads against — no other abbreviations are accepted. Also documents
  availability_mode values (all/latest/any) via avty_mode, and per-component abbreviations
  exist for every HA platform (alarm, fan, light, vacuum, etc.).'
---

*Suggested by [[corehomeassistantcomponentsmqttdiscoverypy-at-dev-home-assistantcore-github]] — discovery.py imports ABBREVIATIONS/DEVICE_ABBREVIATIONS/ORIGIN_ABBREVIATIONS from abbreviations.py — the ground-truth table of legal abbreviated discovery keys*

core/homeassistant/components/mqtt/abbreviations.py at dev · home-assistant/core · GitHub
