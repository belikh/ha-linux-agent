...
id: cpu-performance-scaling-the-linux-kernel-documentation
tags:
...
summary: 'Kernel CPUFreq admin guide (author Rafael J. Wysocki, Intel, © 2017, current
  for 7.3.0-rc1). CPUFreq has three layers (core, scaling governors, scaling drivers);
  user space interface is /sys/devices/system/cpu/cpufreq/policyX/ per policy object,
  where a policy = set of CPUs sharing hardware P-state control. Generic sysfs attributes:
  affected_cpus, bios_limit, cpuinfo_cur_freq (hardware-reported KHz), cpuinfo_avg_freq
  (ARM AMU, ms timeframe), cpuinfo_min/max_freq, cpuinfo_transition_latency, related_cpus,
  scaling_available_frequencies, scaling_available_governors, scaling_cur_freq (last
  requested, may differ from actual), scaling_driver, scaling_governor (read-write
  — write a governor name to switch), scaling_max_freq/scaling_min_freq (read-write
  clamps, min must not exceed max), scaling_setspeed (userspace governor only). Governors:
  performance (max), powersave (min), userspace (manual via scaling_setspeed), schedutil
  (scheduler-integrated, PELT-based, f = 1.25 * f_0 * util / max, IO-wait boosting,
  rate_limit_us tunable, replacement for ondemand/conservative), ondemand and conservative
  (legacy workqueue-based with up_threshold/sampling_rate/freq_step tunables). intel_pstate
  driver bypasses the governor layer with its own algorithms. Global boost knob at
  /sys/devices/system/cpu/cpufreq/boost (0/1); legacy AMD cpb knob is per-policy-looking
  but actually system-wide and deprecated. Relevant to a Linux agent: read scaling_cur_freq/scaling_driver/scaling_governor
  for CPU telemetry; write scaling_governor/scaling_min_freq/scaling_max_freq for
  performance control, with per-policy granularity.'
---
...
under /sys/devices/system/cpu/cpuY/ (where Y represents an integer
...
in /sys/devices/system/cpu/cpufreq each contain policy-specific
...
/sys/devices/system/cpu/cpufreq/.  In either case the name of the
...
LWN.net article [1] for a description of the PELT mechanism).  Then, the new
...
“IO-wait boosting”.  That happens when the SCHED_CPUFREQ_IOWAIT flag
...
This governor generally is regarded as a replacement for the older ondemand
and conservative governors (described below), as it is simpler and more
...
(drivers/cpufreq/amd_freq_sensitivity.c), between 0 and 1000
inclusive.
...
It estimates the CPU load in the same way as the ondemand governor described
...
This file is located under /sys/devices/system/cpu/cpufreq/ and controls
...
sysfs (/sys/devices/system/cpu/cpufreq/policyX/) and is called
...
CONFIG_X86_ACPI_CPUFREQ_CPB configuration option) and the global
...
Jonathan Corbet, Per-entity load tracking,
