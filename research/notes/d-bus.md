...
summary: Chimera Linux documentation explaining the system-vs-session D-Bus split
  and how the session bus is reached. Under systemd (and Chimera's dinit with implicit
  user-service support), the session bus is managed by the user's service manager
  and always lives at DBUS_SESSION_BUS_ADDRESS=unix:path=/run/user/1000/bus, shared
  across all logins/TTYs/desktops thanks to logind. On non-systemd distros without
  user services the bus is launched per-session by dbus-launch/dbus-run-session, producing
  abstract sockets like unix:abstract=/tmp/dbus-... that other sessions cannot see.
  Explicitly warns never to run dbus-run-session inside an environment that already
  has a global session bus — it creates a nested bus invisible to the rest of the
  session. Explains why an agent needing the session bus on a systemd host (NixOS)
  should rely on the user-manager bus at /run/user/<uid>/bus rather than launching
  its own dbus.
---
