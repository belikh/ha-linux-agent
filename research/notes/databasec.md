...
- gap-07
- broker-config
created: '2026-09-02T17:03:39.606813Z'
updated: '2026-09-02T17:39:23.928270Z'
source: https://raw.githubusercontent.com/eclipse-mosquitto/mosquitto/master/src/database.c
source_domain: raw.githubusercontent.com
fetched_at: '2026-09-02T17:03:34.687118Z'
...
content_type: code
deprecated: false
summary: 'mosquitto src/database.c (master, raw C source): the drop mechanism in code.
  db__message_write_queued_out: when a message fails db__ready_for_flight and db__ready_for_queue,
  the branch logs NOTICE ''Outgoing messages are being dropped for client %s'' once
  (context->is_dropping latch), increments mosq_counter_mqtt_publish_dropped and context->stats.messages_dropped,
  returns rc=2 — the message is discarded with no client-visible error. db__ready_for_queue:
  queue admission is per-client (msg_data = context->msgs_out), counted as queued_count
  vs db.config->max_queued_messages (default 1000) and queued_bytes vs max_queued_bytes
  (first-limit-wins); for a DISCONNECTED client, inflight adjustments zero out (adjust_count
  = 0 when !net__is_connected), so the full per-client queue depth is available. Critically
  for QoS 0 (the #135266 case — HA subscribes ''#'' at QoS 0): db__ready_for_flight''s
  QoS-0 branch checks context->out_packet_count < max_queued_messages, so QoS-0 traffic
  to a slow/offline reader is ALSO capped by max_queued_messages. Pins the mechanism:
  the binding quantity is per-client queued messages vs broker-wide retained inventory,
  and QoS 0 is not exempt.'
---

/*
Copyright (c) 2009-2021 Roger Light
