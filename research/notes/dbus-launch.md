...
deprecated: false
summary: 'Upstream dbus-launch(1) man page: the historical X11-session tool that starts
  a session bus and sets DBUS_SESSION_BUS_ADDRESS in the environment. Documents --sh-syntax/--csh-syntax/--binary-syntax
  address output, --exit-with-session lifetime binding, and that dbus-launch is designed
  for one-per-login-session use in X clients'' startup scripts. Superseded on modern
  systemd systems where the session bus is a systemd user service whose address is
  exported as /run/user/1000/bus (via the dbus.socket/dbus.service user units), but
  still the canonical reference for how DBUS_SESSION_BUS_ADDRESS gets populated in
  a classic session. Supports the parent note''s chain: a system-level service does
  not inherit this variable — it is only set inside the user''s login/systemd --user
  environment.'
---
...
or
~/.Xclients.
To start a D-Bus session within a text-mode session,
...
--auto-syntax
Choose --csh-syntax or --sh-syntax based on the SHELL environment variable.--binary-syntax
Write to stdout a nul-terminated bus address, then the bus PID as a
binary integer of size sizeof(pid_t), then the bus X window ID as a
binary integer of size sizeof(long). Integers are in the machine's
byte order, not network byte order or any other canonical byte order.--close-stderr
...
your application.--config-file=FILENAME
Pass --config-file=FILENAME to the bus daemon, instead of passing it
the --session argument. See the man page for dbus-daemon--csh-syntax
Emit csh compatible code to set up environment variables.--exit-with-x11
...
the X session has ended.--exit-with-session
...
use it manually. It may change in the future.--sh-syntax
