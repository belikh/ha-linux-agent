...
created: '2026-09-02T10:33:24.579868Z'
updated: '2026-09-02T17:37:22.479947Z'
source: https://raw.githubusercontent.com/bytebeamio/rumqtt/main/rumqttc/src/eventloop.rs
source_domain: raw.githubusercontent.com
fetched_at: '2026-09-02T10:33:24.578378Z'
...
deprecated: false
summary: 'rumqttc eventloop.rs source (bytebeamio/rumqtt main) — answers the subscribe-before-connect
  question definitively. EventLoop holds: mqtt_options (pub, user-editable between
  connections), state: MqttState (pub), a flume bounded request channel (requests_tx/requests_rx,
  capacity = Client::new cap), and ''pub pending: VecDeque'' — pending packets from
  last session. poll(): when network is None it (re)connects with timeout, then ''if
  !connack.session_present { self.pending.clear(); }'' — clean sessions drop pending;
  persistent sessions resume them. select() dispatches ''Self::next_request(&mut self.pending,
  &self.requests_rx, ...)'' which pops pending first (with pending_throttle sleep)
  else rx.recv_async() — so requests enqueued via client.subscribe() BEFORE first
  connect sit in the bounded flume channel and are drained after CONNACK. clean()
  moves state''s unacked publishes + pending PubRels into eventloop.pending, drains
  the channel too, retains everything EXCEPT Request::PubAck (dropped to avoid confusing
  the broker with unexpected acks), and nulls network/keepalive. Doc comment: ''Continuing
  to poll will reconnect to the broker if there is a disconnection'' and ''Don''t
  block this while iterating''. Flow control: new user requests are only pulled when
  inflight < max_inflight and no pkid collision; collision parks the publish and stops
  new outgoing requests until the right ack arrives. Note the doc recommendation:
  for clean() users, set AsyncClient channel capacity to 0 to bound the pending list.'
---

*Suggested by [[staters]] — state.rs shows outgoing_subscribe has no pre-connection buffering at the state layer; eventloop.rs is where the request channel drains, answering whether subscribe-before-connect is queued*
