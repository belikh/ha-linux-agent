---
title: Generic Thermal Sysfs driver How To — The Linux Kernel  documentation
id: generic-thermal-sysfs-driver-how-to-the-linux-kernel-documentation
tags:
...
summary: 'Kernel generic thermal sysfs driver HOWTO (Sujith Thomas & Zhang Rui, Intel,
  © 2008; current for 7.3.0-rc1). Userspace thermal interface lives under /sys/class/thermal:
  thermal_zone[0-*] directories expose type, temp (millidegrees, e.g. 37000 = 37°C),
  mode, policy (governor, e.g. step_wise), available_policies, trip_point_[0-*]_temp/_type/_hyst,
  emul_temp, plus PID tuning knobs (sustainable_power, k_po/k_pu/k_i/k_d, integral_cutoff,
  slope, offset). cooling_device[0-*] expose type, max_state, cur_state, and stats/
  (time_in_state_ms, total_trans, trans_table — cooling statistics). Zone-to-cooling-device
  binding via cdev[0-*], cdev[0-*]_trip_point, cdev[0-*]_weight. A parallel hwmon
  interface appears under /sys/class/hwmon/hwmon[0-*] (name, temp[1-*]_input, temp[1-*]_critical)
  when no_hwmon is false. Critical trip crossing triggers orderly power-off/reboot,
  then forced, then emergency_restart() — thermal framework shuts the box down itself.
  For an agent: temp + trip points give thermal telemetry; cur_state/max_state give
  fan/throttle state; the thermal core''s own shutdown behaviour bounds what agent-level
  alerting can add.'
---
...
a pointer to a function that reads the
sensor temperature. This is mandatory
callback provided by sensor driver.

set_trips

a pointer to a function that sets a
...
a pointer to a function that sets
...
Please read ABI file testing/sysfs-class-thermal for thermal
