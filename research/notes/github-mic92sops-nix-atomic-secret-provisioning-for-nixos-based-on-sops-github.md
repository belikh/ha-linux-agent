---
title: 'GitHub - Mic92/sops-nix: Atomic secret provisioning for NixOS based on sops
  · GitHub'
id: github-mic92sops-nix-atomic-secret-provisioning-for-nixos-based-on-sops-github
tags:
...
content_type: code
deprecated: false
summary: 'Mic92/sops-nix README (upstream, NixOS secrets provisioning via sops): system
  module decrypts secrets to non-persistent /run/secrets at activation; per-secret
  ownership controls sops.secrets.<name>.owner/.group (e.g. owner = config.users.users.<agent>.name)
  so a secret''s file owner matches the service user; restartUnits = ["home-assistant.service"]
  restarts or reloads units when a secret changes or is newly initialised. Ordering:
  sops-nix must run after NixOS creates users; neededForUsers = true handles the chicken-and-egg
  where a secret (e.g. a user password) is needed BEFORE users are created — those
  secrets cannot have an owner set. Home-manager module subset: runs as a systemd
  USER service (sops-nix.service) rather than an activation script, decrypts into
  the user''s non-persistent /run/user/1000/secrets.d, requires home.homeDirectory;
  consumer user services must order after it (systemd.user.services.mbsync.Unit.After
  = ["sops-nix.service"]). Load-bearing for fleet credential delivery per host class:
  system services get /run/secrets with per-secret owner/restartUnits wiring; user
  services get XDG_RUNTIME_DIR-scoped secrets decrypted only when the user''s service
  manager runs (i.e. with linger for headless hosts) — a direct input to the per-host-class
  credential model.'
---

GitHub - Mic92/sops-nix: Atomic secret provisioning for NixOS based on sops · GitHub
...
Secrets are decrypted from sops files during
...
The other method is age which is based on age.
The tool (ssh-to-age) can convert SSH host or user keys in Ed25519
format to age keys.
...
While the sops-nix system module decrypts secrets to the system non-persistent /run/secrets, the home-manager module places them in the users non-persistent $XDG_RUNTIME_DIR/secrets.d.
...
This example shows the flake approach from the recommended example Install: Flakes (current recommendation)
...
If you have used pass before (e.g. in
krops) than you can use the following one-liner
...
The nix-community infra makes extensive usage of sops-nix.
Each host has a secrets.yaml containing secrets for the host.
Also Samuel Leathers explains his personal setup in this blog article.
...
agenix: Similar features as sops-nix but
uses age.

scalpel: Provides a simple template
...
Topics
build-with-buildbothacktoberfestnixossops
Resources
Readme
MIT license
Activity
