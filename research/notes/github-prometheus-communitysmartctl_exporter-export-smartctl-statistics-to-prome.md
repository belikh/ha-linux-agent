---
title: 'GitHub - prometheus-community/smartctl_exporter: Export smartctl statistics
  to prometheus · GitHub'
id: github-prometheus-communitysmartctl_exporter-export-smartctl-statistics-to-prome
tags:
...
content_type: code
deprecated: false
summary: 'prometheus-community/smartctl_exporter (585 stars, 134 forks, 325 commits,
  Go): wraps smartctl JSON output (requires smartmontools >= 7.0 for JSON export)
  and exposes SMART/NVMe metrics at :9633/metrics. Flags: --smartctl.interval=60s
  poll, --smartctl.rescan=10m device rescan (skipped if any --smartctl.device pinned;
  <1s disables rescan), device-include/exclude regexps, --web.systemd-socket for socket
  activation, --web.config.file for TLS/basic-auth (exporter-toolkit format), repeatable
  listen addresses. Docker requires privileged: true, user: root - block-device access
  mandatory. Explicit data-fidelity policy: exporter does NOT modify smartctl data
  in flight; missing/incorrect data is reported upstream to smartmontools, not patched
  by the exporter. Solid primary-source confirmation of the incumbent design ha-linux-agent
  displaces: external scrape server + root-privileged exporter process per host, versus
  an unprivileged MQTT-pushing agent with HA doing the storage/alerting.'
---

*Suggested by [[best-nvme-ssd-health-monitoring-tools-9-options-ranked]] — the incumbent Prometheus storage-health exporter named in the comparison*

GitHub - prometheus-community/smartctl_exporter: Export smartctl statistics to prometheus · GitHub
...
smartctl valid
smartctl missing
smartctl invalid/incorrect

smartctl_exporter valid
all good
N/A
N/A

smartctl_exporter missing
issue for smartctl_exporter
report upstream to smartmontools
report upstream to smartmontools

smartctl_exporter invalid
issue for smartctl_exporter
issue for smartctl_exporter and report upstream
report upstream to smartmontools
...
Run scripts/collect-smartctl-json.sh to export all drives to a
smartctl-data directory (created in the current directory).

Run scripts/redact_fake_json.py to redact sensitive data.

Provide the JSON file for the drive in question.
