...
id: github-z-galaxyzbus-rust-d-bus-crate-github-2
tags:
...
content_type: code
deprecated: false
summary: 'Fetch of the canonical upstream zbus repo via https://github.com/dbus2/zbus
  — GitHub served the same ''z-galaxy/zbus'' page (dbus2/zbus appears to be a rename/redirect
  of the z-galaxy org), so content is identical to the batch''s z-galaxy note apart
  from one addition: the Portability section states ''Supported targets include Unix,
  Windows and macOS with Linux as the main target. Integration tests of zbus crate
  currently require a session bus running on the build host.'' That last clause is
  directly load-bearing for jupiterOS fleet deployment: any ha-linux-agent D-Bus integration
  test inherits zbus''s own constraint of needing a session bus on the CI host. Also
  documents the alternative dbus-rs crate (libdbus C library) and the #zbus:matrix.org
  support room.'
---

*Suggested by [[github-z-galaxyzbus-rust-d-bus-crate-github]] — assigned URL is the z-galaxy mirror; chasing the canonical upstream repo (dbus2 org)*
