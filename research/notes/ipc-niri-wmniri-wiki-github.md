---
title: IPC · niri-wm/niri Wiki · GitHub
id: ipc-niri-wmniri-wiki-github
tags:
...
summary: 'niri IPC wiki (niri-wm/niri, edited 2026-02-10): niri msg --json is a thin
  wrapper over a UNIX domain socket at $NIRI_SOCKET. Protocol: single-line JSON request
  + newline (or flush+shutdown), single-line JSON reply; event streams (Request::EventStream)
  push full current state up-front then deltas — ''your state can never desync from
  niri''. Two-socket pattern needed to read events AND send requests concurrently.
  Backwards compat: JSON output stable (fields/variants additive only); niri-ipc crate
  follows niri''s version, not semver. The wiki never states the socket''s absolute
  path — that requires source (see server.rs note) — but the socat example shows $NIRI_SOCKET
  is directly connectable by any process that can read the path and has permission.'
---

IPC · niri-wm/niri Wiki · GitHub
