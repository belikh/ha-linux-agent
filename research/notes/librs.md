...
deprecated: false
summary: 'PRIMARY SOURCE: rumqttc lib.rs at main (raw fetch, ~30KB). MqttOptions defaults:
  clean_session=TRUE, keep_alive=60s, request_channel_capacity=10, inflight=100, pending_throttle=0us,
  manual_acks=false, max packet sizes 10KB. set_clean_session(false) is the only path
  to broker-held session state (''broker will hold the client state and performs pending
  operations on the client when reconnection with same client_id happens. Local queue
  state is also held to retransmit packets after reconnection''); PANICS if client_id
  is empty — persistent sessions REQUIRE a stable non-empty client_id. Critical absence:
  NO MQTT v5 support in this file — no session_expiry_interval, no MqttVersion enum;
  eventloop.rs imports mqttbytes::v4 only, so rumqttc at main is MQTT v3.1.1 (session
  persistence only via clean_session=false, broker-side session expiry not negotiable).
  Combined with eventloop.rs: default rumqttc = clean_session=true → every reconnect
  discards pending queue (session_present=false → pending.clear()) and loses all subscriptions.
  A reliable fleet agent MUST set_clean_session(false) with a stable client_id AND
  re-subscribe defensively on every ConnAck. Other API surface: set_pending_throttle
  (outgoing rate), set_inflight (concurrent in-flight, non-zero assert), set_manual_acks,
  NetworkOptions (conn_timeout default 5s, tcp_nodelay, buffer sizes, bind_device
  on Linux, bind_addr), LastWill supported via last_will field.'
---

*Suggested by [[automatic-reconnect-and-subscribed-topics-issue-250-bytebeamiorumqtt-github]] — issue #250 about lost subscriptions cites this source; lib.rs holds MqttOptions clean_session semantics*
...
#[cfg(feature = "use-rustls-no-provider")]
impl From for TlsConfiguration {
...
tcp_send_buffer_size: Option,
tcp_recv_buffer_size: Option,
tcp_nodelay: bool,
conn_timeout: u64,
...
/// ```
pub fn new, T: Into>(id: S, host: T, port: u16) -> MqttOptions {
MqttOptions {
...
/// Security options
pub fn credentials(&self) -> Option {
...
#[cfg(feature = "websocket")]
pub fn set_request_modifier(&mut self, request_modifier: F) -> &mut Self
where
...
#[cfg(feature = "websocket")]
pub fn request_modifier(&self) -> Option {
...
#[cfg(feature = "url")]
impl std::convert::TryFrom for MqttOptions {
type Error = OptionError;

fn try_from(url: url::Url) -> Result {
...
let mut queries = url.query_pairs().collect::>();
...
.map(|v| v.parse::().map_err(|_| OptionError::KeepAlive))
.transpose()?
{
...
.map(|v| v.parse::().map_err(|_| OptionError::CleanSession))
.transpose()?
{
...
.map(|v| v.parse::().map_err(|_| OptionError::PendingThrottle))
.transpose()?
{
...
.remove("inflight_num")
.map(|v| v.parse::().map_err(|_| OptionError::Inflight))
.transpose()?
{
