---
title: local-derivation-goal.cc
id: local-derivation-goalcc
tags:
...
content_type: code
deprecated: false
summary: 'Nix 2.19.2 src/libstore/build/local-derivation-goal.cc (raw C++ source):
  source-level proof that the Linux build sandbox brings up a working private loopback
  interface. In the sandbox setup comment: ''The private network namespace ensures
  that the builder cannot talk to the outside world (or vice versa). It only has a
  private loopback interface. (Fixed-output derivations are not run in a private network
  namespace to allow functions like fetchurl to work.)'' In runChild(): when privateNetwork
  is set, Nix explicitly initialises lo — strcpy(ifr.ifr_name, "lo"); ifr.ifr_flags
  = IFF_UP | IFF_LOOPBACK | IFF_RUNNING; ioctl(fd, SIOCSIFFLAGS) — throwing ''cannot
  set loopback interface flags'' on failure. The child is cloned with CLONE_NEWPID|CLONE_NEWNS|CLONE_NEWIPC|CLONE_NEWUTS|CLONE_PARENT|SIGCHLD
  plus CLONE_NEWNET when privateNetwork. So every sandboxed non-fixed-output build
  on Linux has an UP loopback interface inside its own netns: a mosquitto-subprocess
  harness inside craneLib.cargoTest can bind 127.0.0.1:0, spawn the daemon, and TCP-probe
  readiness — no __noChroot, no runNixOSTest escape needed.'
---
...
{
auto p = dynamic_cast(&worker.store);
assert(p);
return *p;
}
...
autoDelChroot = std::make_shared(chrootRootDir);
...
if (useChroot && settings.preBuildHook != "" && dynamic_cast(drv.get())) {
...
auto ss = tokenizeString>(readLine(sendPid.readSide.get()));
assert(ss.size() == 1);
pid = string2Int(ss[0]).value();
...
StringSet passAsFile = tokenizeString(getOr(drv->env, "passAsFile", ""));
for (auto & i : drv->env) {
...
RestrictedStore(const Params & params, ref next, LocalDerivationGoal & goal)
: StoreConfig(params)
...
auto info = std::make_shared(*next->queryPathInfo(path));
info->deriver.reset();
...
void buildPaths(const std::vector & paths, BuildMode buildMode, std::shared_ptr evalStore) override
{
...
virtual std::optional getBuildLogExact(const StorePath & path) override
{ return std::nullopt; }

virtual void addBuildLog(const StorePath & path, std::string_view log) override
{ unsupported("addBuildLog"); }

std::optional isTrustedClient() override
{ return NotTrusted; }
};
...
ref(std::dynamic_pointer_cast(worker.store.shared_from_this())),
*this);
...
std::map> outputReferencesIfUnregistered;
std::map outputStats;
...
std::optional referencesOpt = std::visit(overloaded {
[&](const AlreadyRegistered & skippedFinalPath) -> std::optional {
finish(skippedFinalPath.path);
return std::nullopt;
},
[&](const PerhapsNeedToRegister & r) -> std::optional {
return r.refs;
},
}, *orifu);
...
void LocalDerivationGoal::checkOutputs(const std::map & outputs)
{
...
std::optional maxSize, maxClosureSize;
std::optional allowedReferences, allowedRequisites, disallowedReferences, disallowedRequisites;
};
...
auto checkRefs = [&](const std::optional & value, bool allowed, bool recursive)
{
if (!value) return;
...
checks.maxSize = maxSize->get();

if (auto maxClosureSize = get(*output, "maxClosureSize"))
checks.maxClosureSize = maxClosureSize->get();

auto get_ = [&](const std::string & name) -> std::optional {
...
- [[nixconf-nix-2352-reference-manual]]
- [[release-11-2012-07-18-nix-reference-manual]]
- [[single-file-cli-init-at-1149-by-n8henrie-pull-request-283878-nixosnixpkgs-github]]
