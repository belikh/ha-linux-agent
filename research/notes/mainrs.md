---
title: switch-to-configuration-ng main.rs (nixpkgs master) — user-unit restart pass
  source
id: mainrs
tags:
...
summary: 'PRIMARY SOURCE CODE (nixpkgs master, ~9.8k words, Rust): switch-to-configuration-ng''s
  user-unit handling. The parent enumerates users via logind.list_users(), then for
  each uid stats /run/user/{uid} (''/run/user/ exists iff the user manager is active
  (BindsTo= on user@.service), so stat() is an atomic, race-free liveness check'')
  and skips with ''skipping user {name}: /run/user/{uid} not found (user manager not
  running)'' when absent — a lingered user with an active manager IS processed. For
  each live user manager it spawns a setuid child (env_clear, XDG_RUNTIME_DIR set)
  running do_user_switch: full daemon-reload + stop/reload/RESTART passes over units
  loaded from /etc/systemd/user (restart_unit calls present), nixos-activation.service
  restarted unconditionally. Two-pass migration logic brings home-manager-shadowed
  or package-shipped units under /etc ownership (restart or start on migration). Root''s
  own user manager: ''Performing user switch for {name}'' handled in same loop. Confirms
  nixos-rebuild switch on 2026 unstable DOES restart user units for any user whose
  manager is running (logged-in OR lingered).'
---
...
static TEMPLATE_UNIT_RE: LazyLock = LazyLock::new(|| {
Regex::new(r"^(.*)@[^\.]*\.(.*)$").expect("systemd template-unit regex is valid")
});

// Matches a unit name, capturing everything up to the type suffix.
static UNIT_NAME_RE: LazyLock = LazyLock::new(|| {
Regex::new(r"^(.*)\.`[[: lower :]]` (POSIX class, wikilink-escaped)*$").expect("systemd unit-name regex is valid")
});
...
fn required_env(var: &str) -> anyhow::Result {
...
/// Glob for `.d/*.conf`, escaping any glob metacharacters in the
/// path prefix so unit names containing e.g. `\` (from systemd-escape) are
/// matched literally.
fn unit_dropin_glob(unit_path: &Path) -> Result {
...
fn parse_unit(unit_file: &Path, base_unit_path: &Path) -> Result {
...
active_cur: &HashMap,
units_to_stop: &mut HashMap,
units_to_start: &mut HashMap,
units_to_reload: &mut HashMap,
units_to_restart: &mut HashMap,
units_to_skip: &mut HashMap,
) -> Result<()> {
...
fn record_unit(p: impl AsRef, unit: &str) {
...
fn unrecord_unit(p: impl AsRef, unit: &str) {
if ACTION.get() != Some(&Action::DryActivate) {
...
fn map_from_list_file(p: impl AsRef) -> HashMap {
...
// and { options }
fn parse_fstab(fstab: impl BufRead) -> (HashMap, HashMap) {
...
fn unit_is_active(conn: &LocalConnection, unit: &str) -> Result {
...
static ACTION: OnceLock = OnceLock::new();
...
fn remove_file_if_exists(p: impl AsRef) -> std::io::Result<()> {
...
fn unit_file_state(unit_file: impl AsRef) -> Result {
...
old_unit_dir: &Path,
new_unit_dir: &Path,
current_active_units: &HashMap,
units_to_stop: &mut HashMap,
units_to_start: &mut HashMap,
units_to_reload: &mut HashMap,
units_to_restart: &mut HashMap,
units_to_skip: &mut HashMap,
units_to_filter: &mut HashMap,
) -> Result<()> {
...
for entry in unit_dropin_glob(¤t_unit_file)?.flatten() {
...
let user_config_unit_dir: Option = match systemd.environment() {
Err(err) => {
...
let migration_candidates: Vec = current_active_units
.iter()
...
let print_units = |verb: &str, units: &HashMap| {
if units.is_empty() {
return;
}
...
// The per-user activation may have removed ~/.config/systemd/user/
...
// /run/user/ exists iff the user manager is active (BindsTo= on
