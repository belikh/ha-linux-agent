---
title: nixpkgs nixos/modules/module-list.nix — master module registry
id: module-listnix
tags:
...
summary: 'ABSENCE VERIFICATION (nixpkgs master nixos/modules/module-list.nix, the
  registry of all NixOS service modules): zero occurrences of ''go-hass'', ''lnxlink'',
  or ''hass-agent''. The services/home-automation/ directory contains modules for
  home-assistant, esphome, zigbee2mqtt, zwave-js, matter-server, govee2mqtt, evcc,
  ebusd, homebridge etc. — but NO module for either HA Linux-agent incumbent. go-hass-agent
  exists in nixpkgs as a package only (pkgs/by-name/go/go-hass-agent, per mynixos.com);
  lnxlink has no nixpkgs package at all (by-name/ln/lnxlink/package.nix 404s). Neither
  incumbent can be enabled declaratively via services.* on NixOS in 2026.'
---
