...
deprecated: false
summary: 'nixpkgs ships a full Mosquitto NixOS VM test at nixos/tests/mosquitto.nix:
  a server node enabling services.mosquitto with three listeners (port 1888 plaintext
  password auth via password_store/password_file/hashed_store/hashed_file user variants,
  TLS port 1889 with snakeoil certs and require_certificate, anonymous port 1890 with
  allow_anonymous), plus two client nodes that use mosquitto_pub/sub CLI tools; testScript
  waits for mosquitto.service then asserts pub/sub, ACL read/write separation (reader
  can read $SYS/#, writer cannot), TLS client-cert identity, and anonymous publishing.
  Directly cribbable for a Rust MQTT agent''s NixOS test: the broker is configured
  entirely through services.mosquitto settings/listeners/users options and the test
  only needs mosquitto.service readiness plus mosquitto clients in environment.systemPackages.'
---
