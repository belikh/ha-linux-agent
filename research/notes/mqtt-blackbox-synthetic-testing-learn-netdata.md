---
title: MQTT Blackbox | Synthetic Testing | Learn Netdata
id: mqtt-blackbox-synthetic-testing-learn-netdata
tags:
...
summary: 'Netdata docs page for MQTT Blackbox synthetic testing: despite the title,
  this is the go.d prometheus collector configuration reference — metrics are gathered
  by periodically sending HTTP requests to an MQTT Blackbox Exporter (a separate Prometheus
  exporter for MQTT transport monitoring), not by speaking MQTT itself. Documents
  job config (go.d/prometheus.conf YAML): url, timeout (10s default), update_every
  (10s), autodetection_retry, selector allow/deny patterns, fallback_type counter/gauge
  patterns, metric_relabel_configs, profiles (auto/exact/combined/none), max_time_series
  (2000), HTTP auth, TLS (mTLS supported), proxy, virtual nodes. Notable for agent
  design: untyped metrics handling rules (suffix _total → Counter; quantile label
  → Summary; le label → Histogram; rest ignored), and the ''disappearing metrics''
  semantics — expired chart/dimension alerts become REMOVED without recovery notification,
  so an explicit 0 export is needed for reliable alert clearing. UI-based config requires
  a paid Netdata Cloud plan; file config is the automatable path (Ansible-able).'
---
...
Metrics are gathered by periodically sending HTTP requests to MQTT Blackbox Exporter.
...
Install MQTT Blackbox Exporter by following the instructions mentioned in the exporter README.
