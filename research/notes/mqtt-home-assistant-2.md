...
summary: 'Same official HA MQTT integration reference fetched via #mqtt-discovery
  anchor. Near-duplicate of note mqtt-home-assistant (same 60KB page) but retained
  as the discovery-focused read: covers MQTT discovery birth-message-triggered re-publish,
  retained discovery payloads, ghost-entity warning for retained messages, message_expiry_interval
  for command payloads, and availability topic configuration (payload_online/offline,
  availability_mode all/any/latest). Key line for ha-linux-agent design: ''a device
  or service using MQTT discovery must make sure a configuration message is offered
  after the MQTT integration has been (re)started'' — the agent must either retain+expire
  its discovery messages or subscribe to homeassistant/status and re-publish on birth.'
---
