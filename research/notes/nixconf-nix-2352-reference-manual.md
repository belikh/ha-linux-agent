---
title: nix.conf - Nix 2.35.2 Reference Manual
id: nixconf-nix-2352-reference-manual
tags:
...
content_type: docs
deprecated: false
summary: 'Nix 2.35.2 nix.conf manual (nix.dev, current stable docs): the authoritative
  ''sandbox'' setting definition. ''If set to true, builds are performed in a sandboxed
  environment ... In addition, on Linux, builds run in private PID, mount, network,
  IPC and UTS namespaces to isolate them from other processes in the system (except
  that fixed-output derivations do not run in private network namespace to ensure
  they can access the network). The default is true on Linux.'' Also documents sandbox-paths,
  the relaxed mode (fixed-output + __noChroot derivations skip the sandbox), and —
  load-bearing for the harness — the proot-style caveat that sandboxed builds have
  no setuid/capability programs unless bind-mounted, which is why userspace network-interface
  setup in tests would need allow-setuid... impurity escapes, whereas plain loopback
  TCP needs no escape.'
---

nix.conf - Nix 2.35.2 Reference Manual
...
The system-wide configuration file nix.conf in the configuration directory.
...
If NIX_USER_CONF_FILES is set, then each path in the list will be loaded in reverse order.
...
On Unix additionally, nix/nix.conf under each directory in XDG_CONFIG_DIRS
...
If set to true, builtins.warn throws an error when logging a warning.
...
If set to true, Nix ignores the allowSubstitutes attribute in derivations and always attempt to use available substituters.
...
To change this setting, make sure the
auto-allocate-uids experimental feature
is enabled.
For example, include the following in nix.conf:
...
See also the per-store build-dir setting.
...
The URI of the remote store in the format ssh://[username@]hostname[:port].
...
A comma-separated list of Nix system types.
...
The maximum number of builds that Nix executes in parallel on the machine.
Typically this should be equal to the number of CPU cores.

The “speed factor”, indicating the relative speed of the machine as a positive integer or decimal number.
...
A machine is only used to build a derivation if all the features in the derivation’s requiredSystemFeatures attribute are supported by that machine.

A comma-separated list of required system features.

A machine is only used to build a derivation if all of the machine’s required features appear in the derivation’s requiredSystemFeatures attribute.

The (base64-encoded) public host key of the remote machine.
...
Sets the value of the NIX_BUILD_CORES environment variable in the invocation of the builder executable of a derivation.
...
If set to true and the --debugger flag is given, builtins.warn
will enter the debugger like builtins.break.
...
as a right operand in an attribute set update expression,
...
Nix only builds a given store derivation locally when its system attribute equals any of the values specified here or in the system option.
...
Deprecated alias: download-attempts
...
To change this setting, make sure the
configurable-impure-env experimental feature
is enabled.
For example, include the following in nix.conf:
...
is passed to the builder if they are listed in impureEnvVars.
...
This doesn’t affect derivations that have preferLocalBuild = true, which are always built locally.
...
List of search paths to use for lookup path resolution.
This setting determines the value of
builtins.nixPath and can be used with builtins.findFile.

The configuration setting is overridden by the NIX_PATH
...
See NIX_STATE_DIR for details on the environment variable.
...
Can be overridden by the NIX_REMOTE environment variable.
...
Nix only builds a given store derivation locally when its system attribute equals any of the values specified here or in extra-platforms.
