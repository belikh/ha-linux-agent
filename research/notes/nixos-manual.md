...
- nixos-tests
- nixos-modules
created: '2026-09-02T06:41:40.061140Z'
updated: '2026-09-02T17:37:22.741840Z'
source: https://nixos.org/manual/nixos/stable/
source_domain: nixos.org
fetched_at: '2026-09-02T06:41:33.191917Z'
fetch_provider: builtin
status: evergreen
type: note
deprecated: false
summary: 'NixOS 26.05 stable manual (single-page edition, 88.5k words) — the canonical
  reference for declarative NixOS configuration. Load-bearing chapters for the linux-agent
  fleet: (1) Writing NixOS Modules — module structure {config, pkgs, ...} with imports/options/config,
  mkOption/mkIf patterns, utils.escapeSystemdExecArg(s) for Exec* directives; (2)
  Mosquitto — services.mosquitto with listeners, per-listener users + ACLs (pattern
  readwrite #), password-file generation disableable via omitPasswordAuth, TLS listener
  options (cafile/certfile/keyfile, port 8883); default ACL is empty i.e. deny-all;
  (3) NixOS Tests — nixos/tests framework, QEMU VMs via nodes + systemd-nspawn containers
  via containers attribute sets, testScript in Python, runNixOSTest from pkgs.testers
  for out-of-tree projects; (4) nixos-rebuild switch/test/boot/build-vm lifecycle,
  build-vm for testing configs in a VM before deploy. Manual confirms Mosquitto is
  the canonical NixOS MQTT broker module for HA integrations.'
---
