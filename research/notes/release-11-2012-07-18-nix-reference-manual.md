---
title: Release 1.1 (2012-07-18) - Nix Reference Manual
id: release-11-2012-07-18-nix-reference-manual
tags:
...
content_type: docs
deprecated: false
summary: 'Nix release 1.1 notes (2012-07-18, canonical wording carried in every Nix
  manual since, incl. 2.29/2.19 renderings): the original statement of the sandbox''s
  loopback semantics — ''The private network namespace ensures that builders cannot
  talk to the outside world (or vice versa): each build only sees a private loopback
  interface. This also means that two concurrent builds can listen on the same port
  (e.g. as part of a test) without conflicting with each other.'' The parenthetical
  ''(e.g. as part of a test)'' is the explicit upstream endorsement of in-sandbox
  TCP client/server tests, and the same-port guarantee is what makes a bind 127.0.0.1:0
  harness safe under parallel flake check runs.'
---

Release 1.1 (2012-07-18) - Nix Reference Manual
...
Build logs are now compressed using bzip2. The command nix-store -l decompresses them on the fly. This can be disabled by setting
...
import <nix/fetchurl.nix> { url = url; sha256 = "hash"; }. (Shea Levy)
