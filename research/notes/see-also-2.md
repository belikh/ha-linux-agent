...
deprecated: false
summary: 'Canonical DocBook XML source of systemd.service(5) (systemd main branch,
  man/systemd.service.xml). Service unit configuration reference — the [Service] section
  semantics an agent''s helper units need. Load-bearing ExecStopPost= definition:
  ''Additional commands that are executed after the service is stopped. This includes
  cases where the commands configured in ExecStop= were used, where the service does
  not have any ExecStop= defined, or where the service exited unexpectedly'' — runs
  on BOTH clean and failed exits, accepts multiple command lines, and  is unset if
  the main process already exited. ExecStop= vs ExecStopPost= division: ''It is recommended
  to use this setting [ExecStop=] for commands that communicate with the service requesting
  clean termination. For post-mortem clean-up steps use ExecStopPost= instead.'' Also:
  restarts are implemented as stop-then-start, so ExecStopPost runs on every restart
  too. Covers Type= behaviours (notify/oneshot), ExecStartPre/ExecStartPost, RemainAfterExit,
  service templates (instance argument via %i, service@arg.service), and systemd-run
  for transient .service/.scope units — the mechanism for agent-initiated on-the-fly
  units. Note body is flattened DocBook (11k words): a LONG source flagged for potential
  source-analyst delegation. Fetched from raw.githubusercontent.com because freedesktop.org
  man pages are behind an anti-bot wall.'
---

*Suggested by [[how-to-get-notifications-if-a-systemd-unit-fails-help-kde-discuss]] — canonical man page source defining OnFailure= and ExecStopPost= semantics cited in the KDE thread*
...
When the service first sends READY=1 during initial startup. If no
signal handler is detected at this point, the service startup is aborted.

Immediately before sending the reload signal. If the handler is no longer present,
...
of the daemon, but also synchronously waits for it to complete. For example, dbus-broker1
uses the following:
...
systemd.kill5). The core file will
...
A service with the <varname>SuccessExitStatus=</varname> setting
...
A service with the <varname>RestartPreventExitStatus=</varname> setting
...
supporting the Live Update Orchestrator,
so that compatible file descriptors (such as memfd_create2)
are preserved and handed back to the service on the other side. See the File Descriptor Store overview for
details.
...
started the service manager creates a LUO session via the Live Update Orchestrator (LUO,
...
descriptors (such as memfd_create2
...
ExecStart=/bin/echo $ONE $TWO $THREE
This results in /bin/echo being
...
Unless for commands with the special executable prefix :,
...
Services that notify systemd about their initialization
...
the Interface Portability and Stability
Promise, so it can be reimplemented by services with no external dependencies. For a
