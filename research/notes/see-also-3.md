...
- drop-in-pattern
created: '2026-09-02T07:44:57.402966Z'
updated: '2026-09-05T10:51:22.141069Z'
source: https://raw.githubusercontent.com/systemd/systemd/main/man/systemd.unit.xml
source_domain: raw.githubusercontent.com
fetched_at: '2026-09-02T07:44:57.400787Z'
...
deprecated: false
summary: 'Canonical DocBook XML source of systemd.unit(5) (systemd main branch, man/systemd.unit.xml).
  Unit configuration reference — defines the dependency and lifecycle directives.
  Key OnFailure= semantics: ''A space-separated list of one or more units that are
  activated when this unit enters the failed state'' (since v201); OnSuccess= activates
  units when entering the inactive state; OnFailureJobMode= (default ''replace'')
  controls how the triggered jobs are enqueued. CRITICAL FLEET PATTERN documented
  verbatim: a global per-type drop-in /etc/systemd/system/service.d/10-all.conf containing
  ''[Unit] OnFailure=failure-handler@%N.service'' adds a failure-handler dependency
  to ALL service units at once, where failure-handler@.service is a Type=oneshot template
  running ''ExecStart=/usr/sbin/myfailurehandler %i'' — after daemon-reload every
  service gains the dependency, and systemd auto-removes the recursive self-dependency
  the template would otherwise create. This is the systemd-native, zero-polling answer
  to the KDE thread''s question and the mechanism an agent could use (or install)
  for fleet-wide failure capture: one drop-in + one template unit, parameterised by
  %N (unit name). Also covers Wants=/Requires=/BindsTo=, PropagatesStopTo=, drop-in
  merge semantics, %i vs %N specifiers. LONG source (13.5k words, flattened DocBook)
  — flagged for potential source-analyst delegation. Fetched from raw.githubusercontent.com
  (freedesktop bot-wall workaround).'
---

*Suggested by [[see-also-2]] — OnFailure= is defined in systemd.unit(5), not systemd.service(5); the service XML references it as the failure-hook mechanism*
...
where type may be e.g. service or socket,
...
systemd-escape1 command may be
...
/run/systemd/generator.early
Generated units with high priority (see early-dir in systemd.generator7)
...
/run/systemd/generator
Generated units with medium priority (see normal-dir in systemd.generator7)
...
/run/systemd/generator.late
Generated units with low priority (see late-dir in systemd.generator7)
...
https://, file:,
info:, man:. For more
information about the syntax of these URIs, see uri7.
...
measured-os
OS PCR measurements enabled. This is typically equivalent to measured-uki, however may also be set explicitly via the systemd.tpm2_measured_os= kernel command line switch, see kernel-command-line7 for details. The various system services doing boot and runtime measurements are conditioned on this flag.
...
(without =) does not match role=webserver.
See
machine-info5
...
in properties of both units, for example in systemctl show
...
Top level drop-ins with template units
