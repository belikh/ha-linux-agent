---
title: niri src/ipc/server.rs — IPC socket creation source (niri-wm/niri main)
id: serverrs
tags:
...
summary: 'PRIMARY SOURCE (niri src/ipc/server.rs, main): exact socket path formula.
  IpcServer::start() builds socket_name = format!("niri.{wayland_socket_name}.{pid}.sock")
  under socket_dir() = BaseDirs::new().runtime_dir() (i.e. $XDG_RUNTIME_DIR, falling
  back to env::temp_dir() = /tmp if unset). Full path: $XDG_RUNTIME_DIR/niri.wayland-N.PID.sock
  (e.g. /run/user/1000/niri.wayland-1.2474.sock — matches the wiki log line in Xwayland.md).
  Wayland socket name comes from the session''s wayland display; PID suffix makes
  the name unique per compositor instance. UnixListener::bind() with default permissions
  (no chmod/umask manipulation, no SO_PEERCRED uid check anywhere in the file — access
  is gated purely by filesystem permissions of the runtime dir, which is 0700 root-owned-by-user
  per systemd). Unlinked on Drop. Also: EVENT_STREAM_BUFFER_SIZE=64 — slow event-stream
  clients get dropped, a capacity constraint for any fleet agent polling via event
  stream.'
---
...
pub fn ipc_screenshot_taken(&mut self, path: Option) {
let Some(server) = &self.niri.ipc_server else {
return;
};
let mut state = server.event_stream_state.borrow_mut();
