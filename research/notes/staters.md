...
created: '2026-09-02T09:58:28.584246Z'
updated: '2026-09-05T10:51:22.211610Z'
source: https://raw.githubusercontent.com/bytebeamio/rumqtt/main/rumqttc/src/state.rs
source_domain: raw.githubusercontent.com
fetched_at: '2026-09-02T09:58:28.582868Z'
...
deprecated: false
summary: 'rumqttc state.rs source (bytebeamio/rumqtt main) — the MqttState machine
  the eventloop drives. Key answers for subscribe-before-connect safety: state.rs
  itself does NOT buffer subscribe requests — outgoing_subscribe() unconditionally
  assigns next_pkid() and returns Ok(Some(Packet::Subscribe)) with no pre-connection
  queue; buffering happens ONE LAYER UP in the eventloop''s request channel (flume),
  and clean() is what feeds pending requests back. clean() implementation: ''Returns
  inflight outgoing packets and clears internal queues'' — it splits outgoing_pub
  at last_puback+1, take()s every unacked QoS1/2 publish (rotating past-ack order),
  collects pending PubRels from outgoing_rel, clears incoming_pub (QoS2 dedup ids),
  resets await_pingresp/collision_ping_count/inflight, and returns the Vec<Request>
  for the eventloop to requeue. Design comment: ''Same state should be used during
  a connection for persistent sessions while new state should be instantiated for
  clean sessions''. Inflight bookkeeping: fixed-size vec indexed by pkid so bad/out-of-order
  acks aren''t O(n); unsolicited acks error; pkid collision on unacked slot parks
  the publish in self.collision (AwaitAck event) and collision_ping_count>=2 raises
  CollisionTimeout; keepalive via last_incoming/last_outgoing Instants with AwaitPingResp
  error if last ping unacked (which the eventloop uses as the half-open-connection
  detector).'
---
...
fn check_collision(&mut self, pkid: u16) -> Option {
...
fn save_pubrel(&mut self, mut pubrel: PubRel) -> Result {
