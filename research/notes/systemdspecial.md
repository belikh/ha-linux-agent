...
deprecated: false
summary: 'Upstream systemd.special(7) man page: documents special units in both system
  and user manager instances. User-manager special units: default.target is the main
  target of the user service manager (analogous to multi-user.target for the system
  instance, but a real unit not an alias; v242+); capsule@.target (v255+) for capsule
  service managers; passive targets graphical-session.target (active while any graphical
  session runs; services should declare PartOf=graphical-session.target to stop with
  the session), plus exit.target, shutdown.target, sockets.target, timers.target,
  paths.target. System-side: user.slice collects all user processes/services started
  on behalf of a user including the per-user systemd instance, pulled in by systemd-logind.service.
  Relevant to fleet service model: an agent run as a user service hangs off default.target
  and lives inside user.slice; lingering determines whether that manager runs at boot
  without a login session.'
---

systemd.specialIndex ·
...
Added in version 235.basic.target¶
...
boots like on system update, emergency boot…display-manager.service¶
The display manager service. Usually, this should be
aliased (symlinked) to gdm.service or a
similar display manager service.emergency.target¶
...
partially booted and disks have already been remounted read-write.exit.target¶
...
Added in version 186.factory-reset.target¶
...
information.
Added in version 250.factory-reset-now.target¶
...
See Factory Reset for more
information.
Added in version 258.final.target¶
...
sleep.target.
Added in version 239.halt.target¶
...
directly.imports.target¶
A target unit that pulls in all disk image download jobs to execute on system boot. This is
used by
systemd-import-generator(8).
Added in version 258.init.scope¶
This scope unit is where the system and service manager (PID 1) itself resides. It
is active as long as the system is running.
Added in version 235.initrd.target¶
...
bootup(7) for
more discussion.
Added in version 245.initrd-fs.target¶
...
the host OS.
Added in version 199.initrd-root-device.target¶
...
Added in version 230.initrd-root-fs.target¶
systemd-fstab-generator(8)
...
Added in version 199.initrd-usr-fs.target¶
systemd-fstab-generator(8)
automatically adds dependencies of type Before= to the
...
hierarchies.
Added in version 249.kbrequest.target¶
...
so this should be used carefully.kexec.target¶
...
for further details of the operation this target pulls in.local-fs.target¶
systemd-fstab-generator(8)
...
[Install] section.network-online.target¶
...
All mount units for remote network file systems automatically pull in this unit, and order
themselves after it. Note that networking daemons that simply provide
functionality to other hosts (as opposed to consume functionality of other
...
Added in version 200.paths.target¶
...
[Install] section.
Added in version 199.poweroff.target¶
A special target unit for shutting down and powering
off the system.
...
for further details of the operation this target pulls in.remote-cryptsetup.target¶
Similar to cryptsetup.target, but for encrypted
devices which are accessed over the network. It is used for
crypttab(5)
entries marked with _netdev.
Added in version 235.remote-integritysetup.target¶
Similar to integritysetup.target, but for integrity protected devices
which are accessed over the network. It is used for
integritytab(5)
entries marked with _netdev.
Added in version 258.remote-veritysetup.target¶
Similar to veritysetup.target, but for verity
protected devices which are accessed over the network. It is used for
veritytab(5)
entries marked with _netdev.
Added in version 248.remote-fs.target¶
Similar to local-fs.target, but
for remote mount points.rescue.target¶
...
"1", for compatibility with SysV.shutdown.target¶
...
kernel or UPS daemons when power fails.sleep.target¶
...
"parent" slices.
Added in version 229.sockets.target¶
...
Added in version 254.storage-target-mode.target¶
A special target unit that can be booted into that selects the "Storage Target Mode" for
...
invoke other services too that make local disks available via other mechanisms.
Added in version 255.suspend.target¶
A special target unit for suspending the system. This
pulls in sleep.target.swap.target¶
Similar to local-fs.target, but
for swap partitions and swap files.sysinit.target¶
...
bootup(7).
syslog.socket¶
The socket unit syslog implementations should listen
...
Interface document.system-update.target, system-update-pre.target, system-update-cleanup.service¶
...
the machine.
Added in version 186.timers.target¶
A special target unit that sets up all timer units
(see
systemd.timer(5)
for details) that shall be active after boot.
...
Added in version 199.umount.target¶
...
boot.
This may be used to pull in Bluetooth management
daemons dynamically when Bluetooth hardware is found.printer.target¶
This target is started automatically as soon as a
printer is plugged in or becomes available at boot.
This may be used to pull in printer management daemons
dynamically when printer hardware is found.smartcard.target¶
This target is started automatically as soon as a
smartcard controller is plugged in or becomes available at
boot.
This may be used to pull in smartcard management
daemons dynamically when smartcard hardware is found.sound.target¶
This target is started automatically as soon as a
sound card is plugged in or becomes available at
boot.
This may be used to pull in audio management daemons
dynamically when audio hardware is found.usb-gadget.target¶
This target is started automatically as soon as a
...
Added in version 242.tpm2.target¶
...
/dev/mapper/foobar.
Added in version 245.cryptsetup-pre.target¶
...
stopped.
Added in version 215.veritysetup-pre.target¶
This passive target unit may be pulled in by services
...
Added in version 248.first-boot-complete.target¶
...
Added in version 247.getty-pre.target¶
...
all local mounts.network.target¶
...
purpose use network-online.target as described above.network-pre.target¶
...
orders itself after this target, but does not pull it in. Also see Running Services After the Network Is Up for more
information.
Added in version 214.nss-lookup.target¶
...
this target, but not pull it in.nss-user-lookup.target¶
...
special ordering against this target.remote-fs-pre.target¶
...
above).rpcbind.target¶
The portmapper/rpcbind pulls in this target and orders
itself before it, to indicate its availability.ssh-access.target¶
...
used.
Added in version 256.time-set.target¶
...
Added in version 242.time-sync.target¶
...
The service manager automatically adds dependencies of type After= for
this target unit to all timer units with at least one OnCalendar= directive.
...
Table 1. Comparison
time-set.targettime-sync.target"quick" to reach"slow" to reachtypically uses local clock sources, boot process not affected by availability of external resourcestypically uses remote clock sources, inserts dependencies on remote resources into boot processreliable, because localunreliable, because typically network involvedtypically guarantees an approximate and roughly monotonic clock onlytypically guarantees an accurate clockimplemented by systemd-timesyncd.serviceimplemented by systemd-time-wait-sync.service
...
Added in version 206.machine.slice¶
...
units are available:
default.target¶
This is the main target of the user service manager, started by default when the service
...
capsule@.target¶
This is the main target of capsule service managers, started by default, instantiated with
...
"gnome-session.target" pulls in Nautilus as top-level service:[Unit]
Description=User systemd services for GNOME graphical session
Wants=nautilus.service
BindsTo=graphical-session.target
"nautilus.service" gets stopped when the session stops:[Unit]
...
Added in version 234.graphical-session-pre.target¶
...
Added in version 234.xdg-desktop-autostart.target¶
...
-.slice¶
The root slice is the root of the user's slice hierarchy.
It usually does not contain units directly, but may be used to set defaults for the whole tree.
Added in version 247.app.slice¶
...
as well as non-critical services should be placed into this slice.
Added in version 247.session.slice¶
...
Added in version 247.background.slice¶
