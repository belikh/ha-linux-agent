...
summary: 'Official docs.influxdata.com page for the Telegraf MQTT Producer output
  plugin (v1 docs, mirrors the GitHub README with the same config reference): servers
  list (tcp/mqtt/tls/mqtts schemes, TLS and non-TLS cannot be mixed), protocol 3.1.1
  default or 5, Go-template topic ''telegraf/{{ .Tag "host" }}/{{ .Name }}'' with
  Sprig functions and per-segment env vars, QoS 0/1/2 default 2, keep_alive default
  0 — but MUST be non-zero against mosquitto v2.0.12+ or the broker returns ''identifier
  rejected'' (bug eclipse/mosquitto#2117; paho.golang reference default 30), secret-store-backed
  username/password, random client_id if unset, 5s write timeout, TLS CA/cert/key
  + insecure_skip_verify, retain flag, MQTT5 publish properties (content_type, response_topic,
  message_expiry, topic_alias, user_properties). Four layouts: non-batch, batch, field
  (one topic per metric field, value-as-string, ignores data_format, tags/timestamp
  omitted), and homie-v4 (Homie IoT v4.0.0 spec topic trees with mandatory slash-free
  homie_device_name and homie_node_id templates). Documented limitations: devices/nodes
  churn as metrics appear/disappear; lifecycle management ''very limited'' — no dynamic
  MQTT ''will'' possible due to client-library limits, so devices are only marked
  lost on clean Telegraf exit and ''might not change in abnormal aborts''; Homie ID
  conversion lower-cases and hyphen-substitutes names, risking property-topic collisions.
  For ha-linux-agent: the agent must ship non-zero keep-alive, real LWT-based availability,
  and stable entity naming — each is a documented Telegraf gap.'
---
