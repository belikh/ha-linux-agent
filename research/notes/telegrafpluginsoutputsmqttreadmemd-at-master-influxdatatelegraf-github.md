---
title: telegraf/plugins/outputs/mqtt/README.md at master · influxdata/telegraf · GitHub
id: telegrafpluginsoutputsmqttreadmemd-at-master-influxdatatelegraf-github
tags:
...
summary: 'Official Telegraf MQTT Producer output plugin README (master branch): config
  reference for pushing host metrics over MQTT. Key options: servers list (tcp/mqtt/tls/mqtts
  schemes, no mixing TLS/non-TLS), protocol 3.1.1 or 5 (default 3.1.1), Go-template
  topic format ''telegraf/{{ .Tag "host" }}/{{ .Name }}'' with Sprig functions and
  env vars per segment, QoS 0/1/2 (default 2), keep_alive (must be non-zero on mosquitto
  v2.0.12+ due to bug eclipse/mosquitto#2117, else ''identifier rejected''; paho.golang
  reference default 30), username/password with secret-store support, random client_id
  if unset, 5s write timeout, TLS CA/cert/key, retain flag, and four layouts: non-batch,
  batch, field (one topic per metric FIELD, value-as-string, ignores data_format),
  and homie-v4 (Homie IoT v4.0.0 spec with //// topic trees, mandatory slash-free
  homie_device_name and homie_node_id templates). Documented limitations: Homie /
  contents churn as Telegraf metrics appear/disappear; lifecycle management very limited
  — no dynamic MQTT ''will'' possible due to client-library limits, so Homie devices
  are only marked lost on clean Telegraf exit, not on abnormal aborts; field/tag names
  lower-cased and hyphen-substituted per Homie ID spec, risking name collisions. Directly
  relevant to ha-linux-agent: the agent''s MQTT publisher must implement non-zero
  keep-alive, graceful-shutdown LWT publication, and stable topic naming to beat Telegraf''s
  documented weaknesses.'
---

telegraf/plugins/outputs/mqtt/README.md at master · influxdata/telegraf · GitHub
...
337 lines (280 loc) · 14.9 KB

master

/
README.mdCopy pathTop
