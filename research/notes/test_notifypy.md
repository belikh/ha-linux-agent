...
content_type: code
deprecated: false
summary: 'HA core tests/components/mqtt/test_notify.py (dev, raw): upstream test suite
  for the MQTT notify platform, 23 discovery references. DEFAULT_CONFIG = {mqtt: {notify:
  {name: ''test'', command_topic: ''test-topic''}}}. test_discovery_removal_notify
  uses the literal discovery payload ''{ "name": "test", "command_topic": "test_topic"
  }'' published as the component config; test_discovery_update_notify mutates name
  between ''Beer''/''Milk''; the file imports help_test_discovery_broken/update/removal/update_unchanged
  plus entity-device-info and unique-id helpers, so discovery setup, update, removal,
  broken-payload, device-info and abbreviation behaviour are all covered upstream.
  Also asserts notify.send_message publishes to command_topic with the entity state
  set to the send timestamp.'
---
