...
id: users-groups-uids-and-gids-on-systemd-systems
tags:
...
deprecated: false
summary: 'systemd.io official doc ''Users, Groups, UIDs and GIDs on systemd Systems'':
  canonical UID/GID range assignments. System users occupy 1-999 (privilege-separated
  daemons), regular users 1000-60000. systemd special ranges: 61184-65519 for DynamicUser=
  allocations (nss-systemd synthesises userdb records so resolution works without
  /etc/passwd), 60001-60513 systemd-homed users, 60578-60705 dynamic greeter users,
  524288-1879048191 systemd-nspawn container ranges, 2147352576-2147418111 foreign-OS-image
  UIDs. Hard requirements: system users must be resolvable without network (local
  passwd only, remote LDAP/NIS not allowed); tty group must have GID 5 (encoded into
  devpts mount params before NSS is available); NSS is the collision-check/synchronisation
  mechanism for dynamic allocation. For jupiterOS: a static agent system user should
  be allocated in 1-999 via sysusers.d/NixOS users.users; the doc explicitly recommends
  against using the dynamic range for regular system or login users.'
---
...
4294967295, aka “32-bit (uid_t) -1” → This UID is not a valid user ID, as
...
65535, aka “16-bit (uid_t) -1” → Before Linux kernel 2.4 uid_t used to be
...
1…999 → System users. These are users that do not map to actual “human”
...
60578…60705 → UIDs for dynamic greeter users are allocated from this range.
...
(documentation). Thus, multiseat-enabled display
...
userdb Varlink API and dynamically allocate users whenever
...
524288…1879048191 → UID range for systemd-nspawn’s automatic allocation of
...
2147352576…2147418111 → UID range used for foreign OS images. For various
...
Definitely, don’t assign less than 65536 UIDs/GIDs.
...
While it’s fine to assign more than 65536 UIDs/GIDs to a container, there’s
...
Consider allocating UID ranges for containers so that the first UID you assign has the lower 16-bits all set to zero.
...
When picking a UID range for containers, make sure to check NSS first, with
...
systemd-homed by default mounts the home directories it manages with UID mapping applied.
...
60514…60577
0x0000EC62…0x0000ECA1
64
Host users mapped into containers
systemd
systemd-nspawn

60578…60705
0x0000ECA2…0x0000ED21
128
Dynamic greeter users
systemd
nss-systemd

60706…61183
0x0000ED22…0x0000EEFF
478
unused

61184…65519
0x0000EF00…0x0000FFEF
4336
Dynamic service users
systemd
nss-systemd

65520…65533
0x0000FFF0…0x0000FFFD
14
unused
...
524288…1879048191
0x00080000…0x6FFFFFFF
1878523904
Container UID ranges
systemd
nss-systemd

1879048192…2147352575
0x70000000…0x7FFDFFFF
268304384
unused

2147352576…2147418111
