---
title: NixOS manual - What happens during a system switch?
id: what-happens-during-a-system-switchchaptermd
tags:
...
summary: 'NixOS manual chapter (nixpkgs master, current): canonical action order during
  nixos-rebuild switch — stop units, activation script, restart systemd if needed,
  reset-failed, daemon-reload, ''Reload systemd user instances (systemctl --user daemon-reload)'',
  restart sysinit-reactivation.target, reload/restart/start units. Documents STC_DISPLAY_ALL_UNITS=1
  and STC_DEBUG=1 env vars for verbose switch output. Notably the numbered order shows
  user-instance reloading as a first-class step, but the manual''s summary level does
  not spell out the per-user restart pass — that lives in stc-ng source and PR #517768.'
---
