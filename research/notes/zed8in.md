...
deprecated: false
summary: 'zed(8) man page from OpenZFS master (zed.8.in): ZED monitors zevents from
  the ZFS kernel module and runs ZEDLETs matching the zevent class. ZEDLETs live in
  the enabled-zedlets dir (/etc/zfs/zed.d), must be root-owned with no group/other
  write, and are invoked when class/subclass prefixes their filename (''all'' matches
  everything). Zevent nvpars are passed as ZEVENT_-prefixed env vars; shared vars
  go in zed.rc (ZED_ prefix); ZED requires root. SIGHUP rescans zedlets.'
---

*Suggested by [[how-to-monitor-zfs-pool-health-and-status-on-ubuntu]]*
