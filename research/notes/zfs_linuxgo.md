...
deprecated: false
summary: 'node_exporter zfs_linux.go collector: reads ONLY /proc/spl/kstat/zfs (base
  ''spl/kstat/zfs'') — per-subsystem files via linuxPathMap (abdstats, arcstats, dbufstats,
  dmu_tx, dnodestats, fm, vdev_cache_stats, vdev_mirror_stats, xuio_stats, zfetchstats,
  zil), plus glob-based pool paths /*/io, /*/objset-*, and /*/state. Pool STATE comes
  from /proc/spl/kstat/zfs/<pool>/state (parsePoolStateFile maps the lowercase state
  word to one of online/degraded/faulted/offline/removed/unavail/suspended as a zfs_zpool_state
  gauge). It does NOT read a ''pool_state'' file and does NOT touch /dev/zfs or any
  ioctl — all metrics are plain procfs reads, i.e. unprivileged-readable on standard
  systems.'
---
