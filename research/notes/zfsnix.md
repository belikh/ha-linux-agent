...
deprecated: false
summary: 'NixOS services.zfs module (nixpkgs master): options are services.zfs.zed.enableMail
  + services.zfs.zed.settings (attrsOf str/int/bool/list rendered to /etc/zfs/zed.d/zed.rc;
  module injects PATH for zedlets and ZED_EMAIL_PROG/ZED_EMAIL_OPTS when enableMail).
  No zed.enable toggle — zfs-zed.service ships via systemd.packages and is wanted
  by zfs.target whenever ZFS is enabled. autoScrub: enable/interval (monthly)/randomizedDelaySec
  (6h)/pools (empty=all), runs ''zpool scrub -w''. enableMail asserts services.mail.sendmailSetuidWrapper.enable.'
---
